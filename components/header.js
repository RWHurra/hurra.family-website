class THeader extends HTMLElement {
    ...
    static SUBDOMAIN_LINKS = {
        'robert': ['about', 'homelab', 'leisure', 'resume'],
        'www': ['about', 'contact'],
        'default': []
    };

    connectedCallback() {
        const navLinks = THeader.SUBDOMAIN_LINKS[THeader.activeSubdomain] || THeader.SUBDOMAIN_LINKS['default'];
        const dynamicLinksHtml = navLinks.map(link => `
            <a href="/${link}">${link}</a>
        `).join('');
    ...
    static getCurrentSubdomain() {
        ...
        if (isDev) {
            ...
            return parts.length === 3 ? 'www' : parts[0];
        }
